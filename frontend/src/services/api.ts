...
const detectBackendPort = async (): Promise<string> => {
  ...
  // Try to read port from .port file (generated by backend)
  try {
    ...
    if (portResponse.ok) {
      ...
      if (!isNaN(port)) {
        const detectedUrl = `http://localhost:${port}/api`;
        console.log('✅ Port detected from .port file:', detectedUrl);
        return detectedUrl;
      }
    }
  } catch {
    // .port file not available, continue with port scanning
  }

  // Try common ports for backend
  const commonPorts = [3001, 3002, 3003, 3004, 3005, 8000, 8001, 8080, 5000, 5001];
  
  console.log('🔍 Scanning common ports:', commonPorts);
  ...
  for (const port of commonPorts) {
    try {
      ...
      if (response.status === 200 && response.data?.success) {
        const detectedUrl = `http://localhost:${port}/api`;
        console.log('✅ Backend detected at:', detectedUrl);
        return detectedUrl;
      }
    } catch {
